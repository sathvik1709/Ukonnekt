...
public class FeedbackTask extends AsyncTask<Void, Void, ArrayList > {

	FeedbackParameters parameters;
	//URL of the Database Stored on the Server
	String url = "http://omega.uta.edu/~tmm4546/Ukonnekt.php?method=getFeedBack&getFeedBackcallback=";
	
	public FeedbackTask(FeedbackParameters feedbackParameters) {
	...
	public ArrayList readJsonStream(Reader stringReader) throws IOException
	{
	   ...
	   {
		   FeedbackResultParameters feedbackResultParameters = null;
		   ArrayList messages = new ArrayList<FeedbackResultParameters>();

		   reader.beginArray();
		   while (reader.hasNext()) 
		   {
			   feedbackResultParameters = readMessage(reader);
			   messages.add(feedbackResultParameters);
		   }
		   reader.endArray();
	   ...
	   public FeedbackResultParameters readMessage(JsonReader reader) throws IOException 
	   {
		  
		     float teaching = 0.0f;
		     float courseWork = 0.0f;
		     float grading = 0.0f;
		     float gtaSupport = 0.0f;
		     ...
		     while (reader.hasNext()) 
		     {
		       ...
		       if (name.equals("teaching"))
		       {
		    	   teaching = (float) reader.nextDouble();
		       }
		       else if (name.equals("courseWork"))
		       {
		    	   courseWork = (float) reader.nextDouble();
		       } 
		       else if (name.equals("grading"))
		       {
		    	   grading = (float) reader.nextDouble();
		       } 
		       else if (name.equals("gtaSupport"))
		       {
		    	   gtaSupport = (float) reader.nextDouble();
		       }
		       else
		       {
		         reader.skipValue();
		     ...
		     return new FeedbackResultParameters(teaching, courseWork, grading, gtaSupport);		     
		}
